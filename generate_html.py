"""
HTML generátor — ATP + WTA, surface advantage, bukméker odds, edge.
"""
import json
...
def compute_stats(results):
    ...
    return {"total":len(bets),"roi":round(profit/staked*100,1) if staked else 0,"profit":round(profit,2)}


def ss_dots(score, align="left"):
    m = SS_META.get(score, SS_META[3])
    dots="".join(
        f'<span style="width:7px;height:7px;border-radius:50%;display:inline-block;margin:0 1px;'
        f'background:{("#fb923c" if i<=(3-score) else "#1e3a5f") if score<=2 else (("#60a5fa" if i>(3-(5-score)) else "#1e3a5f") if score>=4 else ("#94a3b8" if i==3 else "#1e3a5f"))}"></span>'
        for i in range(1,6))
    lbl = f'<span style="font-size:9px;color:{m["col"]};margin-{"left" if align=="left" else "right"}:3px">{m["icon"]}</span>'
    if align=="left": return f'<div style="display:flex;align-items:center;gap:1px">{lbl}{dots}</div>'
...
def edge_badge(edge):
    ...
    return (f'<span style="font-size:9px;font-weight:700;padding:2px 5px;border-radius:4px;'
            f'background:{bg};border:1px solid {bd};color:{col}">{edge*100:+.1f}%{lbl}</span>')
...
def adv_badge(adv, player_num, surface):
    ...
    return (f'<div style="margin-top:3px;padding:2px 6px;border-radius:4px;'
            f'background:#f59e0b15;border:1px solid #f59e0b35;'
            f'font-size:9px;font-weight:700;color:#f59e0b">⚡ Felületi előny — jobb {sl}</div>')
...
def render_card(m):
    ...
    if not m.get("elo_found"):
        return (f'<div class="card" style="opacity:.5"><div class="ctop">'
                f'<div class="tags"><span class="surf-tag" style="background:{sm["bg"]};border-color:{sm["bd"]};color:{sm["col"]}">{sm["icon"]} {sm["label"]}</span>'
                f'<span class="cat-tag" style="color:{tm["col"]}">{tour} {cat}</span><span class="t-tag">{m.get("time","")}</span></div>{pill}</div>'
                f'<div class="cbody"><div style="font-weight:600">{m["player1"]} <span style="color:#64748b">vs</span> {m["player2"]}</div>'
                f'<div style="font-size:11px;color:#ef4444;margin-top:.3rem">⚠ {m.get("error","Elo N/A")}</div></div></div>')

    p1,p2=m["name1"],m["name2"]; r1,r2=m["r1"],m["r2"]
    c1,h1=m["c_elo1"],m["h_elo1"]; c2,h2=m["c_elo2"],m["h_elo2"]
    ...
    book_row=""
    if bo1 or bo2:
        book_row=(f'<div style="display:flex;align-items:center;justify-content:space-between;'
                  f'background:#0d1e35;border:1px solid #1e3050;border-radius:7px;padding:.35rem .6rem;margin-top:.35rem;gap:.3rem">'
                  f'<div style="display:flex;align-items:center;gap:.35rem">'
                  f'<span style="font-size:9px;color:#64748b">Bukméker</span>'
                  f'<span style="font-size:1.1rem;font-weight:800;color:#e2e8f0">{bo1 or "—"}</span>{edge_badge(e1)}</div>'
                  f'<span style="font-size:9px;color:#64748b">TE odds</span>'
                  f'<div style="display:flex;align-items:center;gap:.35rem">{edge_badge(e2)}'
                  f'<span style="font-size:1.1rem;font-weight:800;color:#e2e8f0">{bo2 or "—"}</span></div></div>')
...
def analyze_matches(matches, elo_players):
    ...
    for m in matches:
        ...
        results.append({**m,
            ...
            "surface_advantage":surface_advantage(r1,r2,surf),
            "elo_found":True,"error":None,"status":m.get("status","upcoming")})
    return results


def stat_row(analyses):
    found    = sum(1 for m in analyses if m.get("elo_found"))
    live_n   = sum(1 for m in analyses if m.get("status")=="live")
    value_n  = sum(1 for m in analyses if (m.get("edge1") or 0)>=0.04 or (m.get("edge2") or 0)>=0.04)
    adv_n    = sum(1 for m in analyses if m.get("surface_advantage"))
    return found, live_n, value_n, adv_n
...
def generate_html(atp_analyses, wta_analyses=None, elo_meta=None, bankroll=1000.0):
    ...
    all_rest = [m for m in atp_analyses+wta_analyses if m.get("status")!="live"]

    af,al,av,aa = stat_row(atp_analyses)
    wf,wl,wv,wa = stat_row(wta_analyses)
    total_value  = av+wv
    total_adv    = aa+wa
    ...
    def section(title, lst, color="#94a3b8"):
        if not lst: return ""
        return f'<div class="sec" style="color:{color}">{title}</div><div class="cards">{cards(lst)}</div>'
...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}}
    ...
    .tour-tabs{{display:flex;gap:.5rem;margin:.5rem 0}}
    .ttab{{flex:1;padding:.5rem;border-radius:8px;text-align:center;font-size:11px;font-weight:700;border:1px solid var(--bd);background:var(--bg2);color:var(--mu)}}
    .ttab.atp{{background:#3b82f618;border-color:#3b82f640;color:var(--ac)}}
    .ttab.wta{{background:#ec489918;border-color:#ec489940;color:var(--pink)}}
...
<div class="statbar">
  <div class="stat"><div class="sl">ATP</div><div class="sv" style="color:var(--ac)">{len(atp_analyses)}</div></div>
  <div class="stat"><div class="sl">WTA</div><div class="sv" style="color:var(--pink)">{len(wta_analyses)}</div></div>
  <div class="stat"><div class="sl">Élő</div><div class="sv" style="color:var(--ye)">{al+wl}</div></div>
  <div class="stat"><div class="sl">⚡ Felületi</div><div class="sv" style="color:#f59e0b">{total_adv}</div></div>
  <div class="stat"><div class="sl">🎯 Value</div><div class="sv" style="color:var(--gr)">{total_value}</div></div>
  <div class="stat"><div class="sl">ROI</div><div class="sv" style="color:{'var(--gr)' if stats['roi']>=0 else 'var(--rd)'}">{stats['roi']:+.1f}%</div></div>
</div>

<div class="legend">
  <strong style="color:var(--tx)">⚡</strong> Felületi előny: jobb ezen a borításon + ez az ő jobb borítása + ellenfélnél fordítva (mind a 3 egyszerre) ·
  <strong style="color:var(--tx)">🎯</strong> Value: edge ≥ 4% ·
  🧱🧱(1) 🧱(2) ⚖(3) 💙(4) 💙💙(5)
</div>

{"<div class='sec' style='color:var(--ye)'>● Élő</div><div class='cards'>" + cards(all_live) + "</div>" if all_live else ""}

{section("ATP — Mai meccsek", [m for m in atp_analyses if m.get("status")!="live"], "var(--ac)")}
{section("WTA — Mai meccsek", [m for m in wta_analyses if m.get("status")!="live"], "var(--pink)")}

</main>
<footer>tennisabstract.com Elo (ATP+WTA) · TennisExplorer odds · Fair odds = 1/P · Nem befektetési tanácsadás.</footer>
</body>
</html>"""
